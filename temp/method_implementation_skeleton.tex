...
quality: at every timestep, agents free per \eqref{eq:free} are matched to
tasks in $\mathcal{Q}_t$ ordered by $r_j$ ascending (oldest first), each
matched greedily to the free agent minimising $d_G(\ell_i(t), s_j)$,
\begin{equation}
...
rather than in expectation. This is a deliberate departure from
\citet{knippenberg2021}, who instead let the policy select an out-of-range
move and have the environment silently reinterpret it as a wait, penalised
after the fact: that approach spends training signal teaching the policy not
to attempt moves already known to be illegal from $N^{+}(\ell_i(t))$, and it
lets the executed action (wait) diverge from the sampled one (move) at
exactly the state the value function most needs to be reliable. Masking
costs nothing extra to obtain: $N^{+}(v)$ is already required to define
$\mathcal{U}(v)$ (\ref{sec:pf:env}), so $M(v,\cdot)$ reuses information the
environment computes regardless, rather than adding a second mechanism.
RLlib's newer multi-agent API does not ship
